function solve() {
    ...
    buttons[0].addEventListener('click', function (event) {
        ...
        for (const arrElement of arr) {
            ...
            image.setAttribute('src', arrElement.img);
            cellImage.appendChild(image);

            const cellName = document.createElement('td');
            const pName = document.createElement('p');
            pName.textContent = arrElement.name;
            cellName.appendChild(pName);

            const cellPrice = document.createElement('td');
            const pPrice = document.createElement('p');
            pPrice.textContent = arrElement.price;
            cellPrice.appendChild(pPrice);

            const cellDecoration = document.createElement('td');
            const pDecor = document.createElement('p');
            pDecor.textContent = arrElement.decFactor;
            cellDecoration.appendChild(pDecor);
